...
def run(data, args):
        ...
        for epoch in range(0, args.epochs):
            ...
            if epoch % 100 == 0:
                print(epoch, 'Acc:', accs['test'], 'AUC_ROC:', auc_rocs['test'], 'F1:', F1s['test'],
                      ...
                      auc_rocs['val'] + F1s['val'] + accs['val'] - args.alpha * (
            ...
            if auc_rocs['val'] + F1s['val'] + accs['val'] - args.alpha * (
                ...
                best_val_tradeoff = auc_rocs['val'] + F1s['val'] + accs['val'] - (
                            tmp_parity['val'] + tmp_equality['val'])
    ...
    return round(np.mean(acc) * 100, 2), round(np.mean(f1) * 100, 2), round(np.mean(auc_roc) * 100, 2), round(
        np.mean(parity) * 100, 2), round(np.mean(equality) * 100, 2)
...
if __name__ == '__main__':
    ...
    for m_name in ['GCN']:
        ...
        with open("{}_{}.csv".format(args.dataset, args.encoder), mode="a", newline="") as file:
            ...
            writer.writerow(
                ['epochs', 'c_lr', 'c_wd', 'e_lr', 'e_wd', 'hidden', '**||**', 'acc', 'f1', 'auc_roc', 'DP', 'EO'])
