...
def evaluate(x, classifier, hp, encoder,encoder1,encoder2, data, args):
    classifier.eval()
    encoder.eval()

    with torch.no_grad():
        h = encoder(data.x, data.edge_index, data.adj_norm_sp)
        h1 = encoder1(data.x, data.edge_index, data.vq_adj_norm_sp)
        h2 = encoder2(data.x, data.edge_index, data.sens_adj_norm_sp)
        output = classifier(h+torch.spmm(data.weight,h1+h2))
...
def evaluate2(x, classifier, hp, encoder,data, args):
    classifier.eval()
    encoder.eval()
    ...
    pred = (output.squeeze() > 0).type_as(data.y)
    pred_val = (output[data.val_mask].squeeze() > 0).type_as(data.y)
    pred_test = (output[data.test_mask].squeeze() > 0).type_as(data.y)
    ...
    return accs, auc_rocs, F1s, paritys, equalitys, res