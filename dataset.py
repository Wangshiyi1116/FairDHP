...
def sys_normalized_adjacency_torch(adj):
    ...
    row_sum = torch.sparse.sum(adj_with_self_loops, dim=1).to_dense()
    ...
    d_inv_sqrt_matrix = torch.sparse_coo_tensor(
        indices=torch.arange(num_nodes).repeat(2, 1).cuda(),
        values=d_inv_sqrt,
        size=(num_nodes, num_nodes)
    ).cuda()

    normalized_adj = torch.sparse.mm(
        torch.sparse.mm(d_inv_sqrt_matrix, adj_with_self_loops), d_inv_sqrt_matrix
    )
...
def homograph_draw(adj, s):
    G = nx.from_scipy_sparse_array(adj)
    nei_bal = {node: 1 for node in G.nodes}
    for node in G.nodes():
        ...
        for nei in neighbors:
            if s[nei] != node_s:
                ...
                pos += 1
        nei_bal[node] = pos / (neg + 1e-3)
    bal_idx = []
    heter_idx = []
    home_idx = []
    for node in G.nodes():
        if nei_bal[node] < 0.999:
            heter_idx.append(node)
        elif nei_bal[node] > 1.0:
            home_idx.append(node)
        else:
            bal_idx.append(node)

    u  =1

def hetergraph_draw(adj , s):

    G = nx.from_scipy_sparse_array(adj)
    connected_components = list(nx.connected_components(G))
    # 输出连通子图的数量
    print(f"图 G 包含 {len(connected_components)} 个子图")

    # 如果只有一个连通组件，说明图没有子图
    if len(connected_components) > 1:
        n = []
        for i, component in enumerate(connected_components, 1):
            if len(component) > 14 and len(component) < 100:
                subnode = component
                ...
                # nx.draw(g, node_color=l, cmap=plt.cm.RdYlBu, node_size=200, font_size=16, font_weight='bold')
                # plt.show()
    else:
        print("图 G 是一个连通图，没有子图。")
    subnode = n
    g = G.subgraph(subnode)
    l = []
    c = []
    ...
    cmap = LinearSegmentedColormap.from_list(
        "custom_cmap", [(0, "black"), (1, "blue")], N=2
    )
    plt.figure(figsize=(18, 18))
    pos = nx.arf_layout(g)
    nx.draw_networkx_nodes(g, pos=pos, node_color=l, cmap=plt.cm.RdYlBu, node_size=400)
    nx.draw_networkx_edges(g, pos=pos, edge_color=c, edge_cmap=plt.cm.Paired ,width=4.0)
    plt.show()
    print(n)
    u=1
...
#    idx_features_labels['TotalMonthsOverdue'] = (idx_features_labels['TotalMonthsOverdue']-idx_features_labels['TotalMonthsOverdue'].mean())/idx_features_labels['TotalMonthsOverdue'].std()
    ...
    # hetergraph_draw(adj,idx_features_labels[sens_attr].values.astype(int))
    homograph_draw(adj, idx_features_labels[sens_attr].values.astype(int))
...
def get_dataset(dataname):
    ...
    adj_norm_sp, edge_index, features, labels, train_mask, val_mask, test_mask, sens, adj = load(dataset=dataname, label_number=label_num)
